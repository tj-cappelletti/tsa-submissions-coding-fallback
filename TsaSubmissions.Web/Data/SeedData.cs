...
public static class SeedData
{
    public static async Task InitializeAsync(IServiceProvider services)
    {
        ...
        if (!await db.Users.AnyAsync())
        {
            ...
            var judge = new AppUser { Username = "judge", Role = AppRole.Judge };
            judge.PasswordHash = hasher.HashPassword(judge, "judge123!");

            var participant = new AppUser { Username = "participant", Role = AppRole.Participant };
            participant.PasswordHash = hasher.HashPassword(participant, "participant123!");

            db.Users.AddRange(judge, participant);
        }
